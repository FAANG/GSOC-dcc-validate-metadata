...
import os
import re
from faang_gsoc.constants import ALLOWED_SHEET_NAMES, \
...
from faang_gsoc.helpers import convert_to_snake_case, \
    get_rules_json
...
class ReadExcelFile:
            ...
            if sh.name not in ALLOWED_SHEET_NAMES:
                if sh.name == 'faang_field_values':
                    ...
                    try:
                        ...
                        continue
                elif sh.name in EXPERIMENT_ALLOWED_SPECIAL_SHEET_NAMES and self.json_type == 'experiments' or self.json_type == 'analyses':
                    ...
                    continue
                elif sh.name in SAMPLES_ALLOWED_SPECIAL_SHEET_NAMES and self.json_type == 'samples':
                    special_sheet_data = self.get_additional_data(
                    ...
                    os.remove(self.file_path)
                    return f"Error: there are no rules for {sh.name} type!", structure
            else:
                tmp = list()
                self.headers = [
                ...
                try:
                    field_names_indexes = self.get_field_names_and_indexes(
                        sh.name)
                    structure[convert_to_snake_case(sh.name)] = field_names_indexes
                ...
                for row_number in range(1, sh.nrows):
                    ...
                    material_consistency = self.check_sheet_name_material_consistency(sample_data, sh.name)
                    ...
                    sc_prj = None
                    if self.json_type == 'samples' and 'secondary_project' in sample_data['samples_core']:
                        sc_prj = sample_data['samples_core']['secondary_project']
                    elif self.json_type == 'experiments' and 'secondary_project' in sample_data['experiments_core']:
                        sc_prj = sample_data['experiments_core']['secondary_project']
                    elif self.json_type == 'analyses' and 'secondary_project' in sample_data:
    ...
    def get_additional_data(table_object, sheet_name,
        ...
        for row_number in range(1, table_object.nrows):
            ...
            for index, additional_field in enumerate(sheet_fields['all']):
                try:
                    tmp[additional_field] = table_object.row_values(row_number)[index]
                except IndexError:
                    if additional_field in sheet_fields['mandatory']:
                        error_field_name = ' '.join(additional_field.split('_'))
                        return f"Error: '{error_field_name}' field is mandatory in '{sheet_name}' sheet"
                if additional_field in sheet_fields['mandatory'] and tmp[additional_field] == '':
                    error_field_name = ' '.join(additional_field.split('_'))
                    return f"Error: '{error_field_name}' field is mandatory in '{sheet_name}' sheet"
            data.append(tmp)
        if len(data) == 0:
    ...
    def get_custom_data_fields(self, field_names, sheet_name):
        ...
        for header in self.headers:
            if header not in headers_to_check and header not in SPECIAL_PROPERTIES:
                ...
                if len(self.headers)-1 > (indexes[0] + 1) and self.headers[indexes[0] + 1] == 'unit':
                    custom_data_fields_indexes[header] = ['value', 'units']
                elif len(self.headers)-1 > (indexes[0] + 1) and self.headers[indexes[0] + 1] == 'term_source_id':
    ...
    def return_all_indexes(self, item_to_check):
        ...
        :return:
        """
        return [index for index, value in enumerate(self.headers) if value == item_to_check]
    ...
    def get_indices(self, field_name, field_types, array_fields):
        ...
        if len(field_types) == 1 and 'value' in field_types:
            ...
            elif (len(indices) > 1 and field_name in array_fields) or (field_name in array_fields):
                indices_list = list()
            ...
            if field_types == ['value', 'units']:
                ...
                if len(value_indices) == 1 and field_name not in array_fields:
                    return self.check_field_existence(value_indices[0],
                                                      field_name, 'value',
                                                      'unit')
                elif (len(value_indices) > 1 and field_name in array_fields) or (field_name in array_fields):
                    indices_list = list()
                ...
                if len(text_indices) == 1 and field_name not in array_fields:
                    return self.check_field_existence(text_indices[0],
                                                      field_name, 'text',
                                                      'term_source_id')
                elif (len(text_indices) > 1 and field_name in array_fields) or (field_name in array_fields):
                    indices_list = list()
    ...
    def get_data(self, input_data, date_field, time_field, **fields):
        ...
        for field_name, field_index in fields.items():
            ...
            if cell_value != '':
                ...
                if field_name == 'term' and isinstance(cell_value, str)  and "_" in cell_value:
                    cell_value = cell_value.replace("_", ":")

                # Convert date data to string (as Excel stores date in float
                # format)
                if date_field is True and isinstance(cell_value, float) and field_name == 'value':
    ...
    def check_sheet_name_material_consistency(self, sample_data, name):
        ...
        if self.json_type != 'samples':
            return False
        if 'samples_core' in sample_data and 'material' in sample_data['samples_core'] and 'text' in sample_data['samples_core']['material']:
            material = sample_data['samples_core']['material']['text']
            if material != name:
                if name in ['teleostei embryo', 'teleostei post-hatching'] and material == 'specimen from organism':
                    return False
                else:
                    return f"Error: '{name}' sheet contains record with inconsistent material '{material}'"
            else:
                return False
        else:
